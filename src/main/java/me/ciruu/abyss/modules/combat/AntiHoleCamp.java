...
extends Module {
    ...
    private void Method530() {
        ...
        if (list.isEmpty()) {
            ...
            if (!(Globals.mc.player.getDistanceSq(blockPos) < Class29.Method114(((Float)this.range.getValue()).floatValue())) || Class31.Method533(blockPos, (Boolean)this.raytrace.getValue()) != 3);
    ...
    private BlockPos Method529() {
        ...
        if (!arrayList2.isEmpty()) {
            for (BlockPos blockPos7 : arrayList2) {
                ...
                if (!(this.currentTarget.getPositionVector().distanceTo(vec3d.add(null)) < d)) continue;
                blockPos5 = blockPos7;
            }
        }
        return blockPos5;
