...
extends Module {
    ...
    public String Method2991() {
        ...
        if (Globals.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING) {
            int n = Globals.mc.player.inventory.mainInventory.stream().filter(OffHand::Method2992).mapToInt(null).sum() + 1;
            string = "Totem(" + n + ")";
        }
    ...
    private void Method2993() {
        if (Class30.Method1454((Entity)Globals.mc.player) > ((Float)this.togglehealth.getValue()).floatValue() && ((Boolean)this.rightclickgap.getValue()).booleanValue()) {
            if (Globals.mc.player.getHeldItemOffhand().getItem() != Items.GOLDEN_APPLE && Globals.mc.gameSettings.keyBindUseItem.isKeyDown() && Globals.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword && this.mode.getValue() != Class96.Gapple) {
                this.Field2486 = (Class96)((Object)this.mode.getValue());
                this.mode.setValue((Object)Class96.Gapple);
                this.Field2487 = true;
                return;
            }
            if (Globals.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && !Globals.mc.gameSettings.keyBindUseItem.isPressed() && this.Field2487) {
        ...
        if (((HoleFiller)Manager.moduleManager.getModuleByClass(HoleFiller.class)).equals(null)) {
