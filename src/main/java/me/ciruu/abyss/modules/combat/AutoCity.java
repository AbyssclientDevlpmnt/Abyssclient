...
extends Module {
    ...
    private final Setting mode = new Setting("1.13Mode", "", this, false);
    ...
    public void Method3884() {
        ...
        if (arrayList.isEmpty() && !((Boolean)this.burrow.getValue()).booleanValue() && !((Boolean)this.mode.getValue())) {
        ...
        if (!arrayList.isEmpty()) {
            while (((Iterator<?>) blockPos).hasNext()) {
                Class307 class307 = (Class307)((Iterator<?>) blockPos).next();
                    if (blockPos2 == null) {
        ...
        if (((Boolean)this.mode.getValue()).booleanValue()) {
    ...
    private void Method3882(EventPlayerUpdateWalking eventPlayerUpdateWalking) {
        ...
        if (!bl) {
            for (int i = 0; i < 9; ++i) {
                ...
                if (object.toString().isEmpty() || object != Items.DIAMOND_PICKAXE) continue;
                bl = true;
