...
extends Module {
    ...
    private void Method2111() {
        ...
        this.Method2112();
        if (((Boolean)this.toggleoffground.getValue()).booleanValue() && !Globals.mc.player.onGround && !Globals.mc.player.onGround) {

            Globals.printChatMessage("Toggling, you are off ground!");
            return;
        }
        if (((Boolean)this.stepfix.getValue()).booleanValue() && Globals.mc.player.onGround && Globals.mc.player.onGround && Globals.mc.player.posY - this.Field1707 > 0.99) {
