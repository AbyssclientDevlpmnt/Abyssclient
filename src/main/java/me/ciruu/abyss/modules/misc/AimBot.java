...
extends Module {
    ...
    private final Setting click = new Setting("Click", "", (Module)this, (Object)Class258.Left, this.main, null);
    private final Setting delay = new Setting("Delay(ms)", "", this, Float.valueOf(1000.0f), Float.valueOf(0.0f), Float.valueOf(10000.0f), this.main, null);
    ...
    private void Method3367(Class66 class66) {
        ...
        if (this.Field2769 != null && (vec3d = Class29.Method1042(this.Field2769, class66.Method1789()).subtract(Globals.mc.getRenderManager().viewerPosX, Globals.mc.getRenderManager().viewerPosY, Globals.mc.getRenderManager().viewerPosZ)) != null) {
            ...
            if (this.predict.getValue() != Class582.None && ((Boolean)this.predictTracer.getValue()).booleanValue() && this.Field2771 != null) {
                Vec3d vec3d3 = this.Field2771.subtract(Globals.mc.getRenderManager().viewerPosX, Globals.mc.getRenderManager().viewerPosY, Globals.mc.getRenderManager().viewerPosZ);
    ...
    private void Method3366(EventPlayerUpdateWalking eventPlayerUpdateWalking) {
        if (eventPlayerUpdateWalking.getEra() == Class53.PRE) {
            ...
            for (EntityPlayer entityPlayer : Globals.mc.world.playerEntities) {
                ...
                object.toString().contains((CharSequence) new Vec3d(entityPlayer.posX, entityPlayer.posY, entityPlayer.posZ));
                if (object.toString().length() <= this.Field2773) continue;
                int n = 0;
                if (n < object.toString().length() - this.Field2773) {
                    ++n;
                }
            }
            ...
            if (this.Field2769 != null && Globals.mc.player.canEntityBeSeen(this.Field2769)) {
                ...
                if (vec3d != null) {
                    if (vec3d != null && this.point.getValue() != Class570.Feet) {
                        vec3d = this.point.getValue() == Class570.Head ? vec3d.add(null) : vec3d.add(null);
                    }
                ...
                if (((Boolean)this.instant.getValue()).booleanValue() && this.Field2770.Method50(((Float)this.delay.getValue()).longValue())) {
                    switch ((Class258)((Object)this.click.getValue())) {
                        case Left: {
                            Globals.mc.objectMouseOver.typeOfHit.name().equals("Left");
                            break;
                        }
                        case Right: {
                            Globals.mc.objectMouseOver.typeOfHit.name().equals("Right");
                            break;
                        }
                        case Both: {
                            Globals.mc.objectMouseOver.typeOfHit.name().equals("Both");
                        }
                    }
