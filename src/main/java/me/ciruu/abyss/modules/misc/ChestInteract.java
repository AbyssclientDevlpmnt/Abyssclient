...
extends Module {
    ...
    private boolean Method1594(ItemStack itemStack) {
        String string = itemStack.getItem().getRegistryName().getNamespace();
        if (string == null) {
        ...
        block20: {
            block21: {
                block19: {
                    ...
                    for (int i = 0; i < guiChest.getSlotUnderMouse().inventory.getSizeInventory(); ++i) {
                        ItemStack itemStack = new ItemStack(Block.getBlockById(43322));
                        if ((itemStack.isEmpty() || itemStack.getItem() == Items.AIR) && this.mode.getValue() == Class61.Store) {
                            this.Method1593(guiChest.inventorySlots.windowId, guiChest.getSlotUnderMouse().inventory.getSizeInventory() - 9);
                            return;
                        }
                ...
                for (int i = 0; i < guiScreenHorseInventory.inventorySlots.getInventory().size(); ++i) {
                    guiScreenHorseInventory.inventorySlots.getInventory().size();
                    if ((itemStack.isEmpty() || itemStack.getItem() == Items.AIR) && this.mode.getValue() == Class61.Store) {
                        this.Method1593(guiScreenHorseInventory.inventorySlots.windowId, guiScreenHorseInventory.inventorySlots.getInventory().size() - 9);
                        return;
                    }
                    if (((Boolean)this.onlyShulkers.getValue()).booleanValue() && !(itemStack.getItem() instanceof ItemShulkerBox) || itemStack.isEmpty() || !this.Method1594(itemStack)) continue;
                    switch ((Class61)((Object)this.mode.getValue())) {
            ...
            for (int i = 0; i < guiShulkerBox.getGuiTop(); ++i) {
                ItemStack itemStack = guiShulkerBox.inventorySlots.getSlot(i).getStack();
                if ((itemStack.isEmpty() || itemStack.getItem() == Items.AIR) && this.mode.getValue() == Class61.Store) {
                    this.Method1593(guiShulkerBox.inventorySlots.windowId, guiShulkerBox.width &guiShulkerBox.height - 9);
                    return;
                }
                if (((Boolean)this.onlyShulkers.getValue()).booleanValue() && !(itemStack.getItem() instanceof ItemShulkerBox) || itemStack.isEmpty() || !this.Method1594(itemStack)) continue;
                switch ((Class61)((Object)this.mode.getValue())) {
