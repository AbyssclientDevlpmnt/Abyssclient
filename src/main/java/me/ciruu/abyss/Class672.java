...
extends Module {
    ...
    private void Method4147(EventNetworkPostPacketEvent eventNetworkPostPacketEvent) {
        ...
        if (eventNetworkPostPacketEvent.getPacket() instanceof CPacketUseEntity && ((Boolean)this.Field3430.getValue()).booleanValue()) {
            eventNetworkPostPacketEvent.cancel();
        }
        if (((Boolean)this.Field3431.getValue()).booleanValue() && eventNetworkPostPacketEvent.getPacket().getClass().getSimpleName().startsWith("f")) {
            eventNetworkPostPacketEvent.cancel();
        }
        if (((Boolean)this.Field3432.getValue()).booleanValue() && eventNetworkPostPacketEvent.getPacket() instanceof CPacketAnimation) {
            eventNetworkPostPacketEvent.cancel();
        }
    }
}
