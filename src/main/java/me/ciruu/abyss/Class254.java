...
implements Listenable {
    ...
    public Vec3d Method966(int n, boolean bl) {
        if (this.Field676 != null && Globals.mc.world != null) {
            ...
            for (int i = 0; i < n; ++i) {
                if (this.Method969((World)Globals.mc.world, this.Field676.getCollisionBoundingBox(), vec3d2.add(vec3d))) {
                    vec3d2.add(vec3d);
                    continue;
                }
                if (!this.Method969((World)Globals.mc.world, this.Field676.getCollisionBoundingBox(), vec3d2.add(null))) break;
                vec3d2.add(null);
            }
            return vec3d2;
        }
