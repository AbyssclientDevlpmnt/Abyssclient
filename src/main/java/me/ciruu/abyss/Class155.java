...
public class Class155 {
    ...
    public static int Method544(Class clazz) {
        for (int i = 0; i < 9; ++i) {
            ...
            if (!(itemStack.getItem() instanceof ItemBlock) || !clazz.isInstance(block = ((ItemBlock) itemStack.getItem()).getBlock()))
                continue;
            return i;
        }
        return -1;
    ...
    public static int Method3000(Block block) {
        int n = -1;
        for (int i = 0; i < 9; ++i) {
            if (Globals.mc.player.inventory.getStackInSlot(i).getItem() != Item.getItemFromBlock((Block) block))
                continue;
            n = i;
        }
        return n;
    ...
    public static int Method3003(Item item, boolean bl) {
        ...
        atomicInteger.set(-1);
            if (((ItemStack) entry.getValue()).getItem() != item || (Integer) entry.getKey() == 45 && !bl);
            atomicInteger.set((Integer) entry.getKey());
            return atomicInteger.get();
    }

    public static Map.Entry slot;

    public static List Method1807(boolean bl) {
        ArrayList<Object> arrayList = new ArrayList<Object>();
        if (!((ItemStack) slot.getValue()).isEmpty() && ((ItemStack) slot.getValue()).getItem() != Items.AIR);
    ...
    public static int Method3005(Class clazz, boolean bl) {
        AtomicInteger atomicInteger = new AtomicInteger();
        atomicInteger.set(-1);
        if (!Class155.Method1094(((ItemStack) entry.getValue()).getItem(), clazz) || (Integer) entry.getKey() == 45 && !bl);
        atomicInteger.set((Integer) entry.getKey());
        return atomicInteger.get();
    }
