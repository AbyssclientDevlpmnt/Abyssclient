...
extends Module {
    ...
    private void Method3873(EventNetworkPostPacketEvent eventNetworkPostPacketEvent) {
        if (eventNetworkPostPacketEvent.getPacket() instanceof CPacketPlayer) {
            if (this.Field3201.contains((CPacketPlayer)eventNetworkPostPacketEvent.getPacket())) {
                this.Field3201.remove((CPacketPlayer)eventNetworkPostPacketEvent.getPacket());
            } else {
    ...
    private void Method3870(EventNetworkPrePacketEvent eventNetworkPrePacketEvent) {
        if (eventNetworkPrePacketEvent.getPacket() instanceof SPacketPlayerPosLook && Globals.mc.player != null && Globals.mc.world != null) {
        ...
        if (((Boolean)this.Field3189.getValue()).booleanValue() && eventNetworkPrePacketEvent.getPacket() instanceof SPacketCloseWindow) {
            eventNetworkPrePacketEvent.cancel();
        }
    }

    private void Method3869(EventPlayerUpdateWalking eventPlayerUpdateWalking) {
        if (eventPlayerUpdateWalking.getEra() == Class53.PRE && this.Field3200) {
