...
public final class Class19 {
    ...
    private void Method4254(EventNetworkPrePacketEvent eventNetworkPrePacketEvent) {
        ...
        String __string = packet.getClass().getSimpleName();
        if (__string.startsWith("C")) {
            return;
        }
        try {
            ...
            if (packet instanceof SPacketDestroyEntities) {
                ...
                for (int n2 : nArray = ((SPacketDestroyEntities) packet).getEntityIDs()) {
                    this.Field65 = Math.max(n2, this.Field65);
                    if (((AutoCrystal) Manager.moduleManager.getModuleByClass(AutoCrystal.class)).Field70 == null || ((AutoCrystal) Manager.moduleManager.getModuleByClass(AutoCrystal.class)).Field70.getEntityId() != n2)
                        continue;
                    Predict.Method602();
                    Predict.Method597();
                }
