...
implements Listenable {
    ...
    public static LinkedList Field1891;
    private Vec3d Field1892;
    private Vec3d Field1893;
    @EventHandler
    public static Listener Field1894;
        ...
        if (worldClient != null) {
            ...
            if (entity != null) {
                ...
                if (n2 <= n3) {
                    while (true) {
                        Vec3d vec3d4;
                        if (this.Method2267((World)worldClient3, entity3.getCollisionBoundingBox(), vec3d3.add(vec3d2))) {
                            vec3d4 = vec3d3.add(vec3d2);
                        } else {
                            if (!this.Method2267((World)worldClient3, entity3.getCollisionBoundingBox(), vec3d3.add(null))) break;
                            vec3d4 = vec3d3.add(null);
                        }
                        vec3d3 = vec3d4;
                        if (n2 == n3) break;
                        ++n2;
    ...
    public final float Method2265() {
        return Globals.mc.isGamePaused() ? Globals.mc.getRenderPartialTicks(): Globals.mc.getRenderPartialTicks();
    }
    ...
    public <DefaultConstructorMarker> Class468(Entity entity, int n, int n2, DefaultConstructorMarker defaultConstructorMarker) {
        if ((n2 & 2) != 0) {
            n = 20;
        }
    }
